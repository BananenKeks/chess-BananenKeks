...
public class Board extends JFrame
{
	...
	MouseAdapter ma = new MouseAdapter(){
		...
		public void mouseDragged(MouseEvent mouseEvent) {
			//ListenerInterrupted should stop this event afterwards. It doesn't work though.
			//TODO fix this!
			if(isListenerInterrupted() == false){
				...
				if(SwingUtilities.isLeftMouseButton(mouseEvent)){
					...
					//ListenerInterrupted is set to true, so this method should not be executed.
					//TODO this leads to the event that you can call this method only once.
		...
		public void mouseClicked(MouseEvent mouseEvent) {
			//Left mouse button is clicked!
			if(SwingUtilities.isLeftMouseButton(mouseEvent)){
				//There is currently no figure selected. This attribute is used to separate a click for picking up a figure and a click for moving a figure.
				//No figure is picked up!
				if(isPickedUp()==false){
					//Asking for the occupation state of the clicked tile and saving that state into class own attribute!
					setTileCurrentlyOccupied(((Tile) mouseEvent.getSource()).isTileCurrentlyOccupied());
						...
						if(isTileCurrentlyOccupied() == false){
							...
							//TODO add proper response or none. This can be done when no console output is needed.
							System.err.println("No Figure!");
						}
						//There is a figure on the field!
						else{
							//Asking for the player state of the clicked tile and saving that state into class own attribute!
							setTileCurrentPlayer(((Tile) mouseEvent.getSource()).getTileCurrentPlayer());
							//Checks if figure on clicked tile belongs to current active player!
							...
							//Clicked figure belongs to active player!
								...
								//Asking for the figure type of the clicked tile and saving that type into class own attribute!
								...
								//Changing state of pickedUp attribute to be able to move the figure on the next click!
								setPickedUp(true);
								//Changing the occupation state of the clicked tile to false. There is not figure anymore!
								((Tile) mouseEvent.getSource()).setTileCurrentlyOccupied(false);
								//Changing the figure type to none. There is no figure anymore!
								//TODO when game is functional then remove the "none" theme!
								((Tile) mouseEvent.getSource()).setTileCurrentFigure("none");
								//Changing state of active player. There is no figure of a player anymore!
				...
				//Algorithm for moving a figure.
					...
					//Set clicked tile occupation state to true. Now there is a figure!
					((Tile) mouseEvent.getSource()).setTileCurrentlyOccupied(true);
					//Set figure type of tile picked up figure. Now there is that figure!
					((Tile) mouseEvent.getSource()).setTileCurrentFigure(getChosenFigure());
					//Set the player state of tile to active player. Now there is a figure from the active player on the tile!
					((Tile) mouseEvent.getSource()).setTileCurrentPlayer(getCurrentActivePlayer());
					//Change state pickedUp to false. Now another figure can be picked up!
					setPickedUp(false);
			...
			//TODO on right click put figure down on tile where it was picked up.
			else{
				//Printing clicked tile!
				//TODO remove this when not needed anymore.
		...
		//Method for mouseEntered event. It colors the tile when the mouse enters.
		@Override
		...
		//Method for mouseExited. It colors the tile back to the original color.
		@Override
		public void mouseExited(MouseEvent mouseEvent) {
			//Set the background of the tile to the original tile color!
			//TODO maybe shorten this.
			((Tile) mouseEvent.getSource()).setBackground(((Tile) mouseEvent.getSource()).getTileColor());
